        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-camture-pink-bg overflow-hidden shadow-xl sm:rounded-2xl p-8">
                ...
                @else
                            ...
                            <a href="{{ route('photo.result', $photo) }}" class="group block text-center transition-transform duration-300 hover:-translate-y-2">
                                ...
                                <div class="aspect-square bg-white p-2 rounded-lg overflow-hidden shadow-lg group-hover:shadow-2xl transition-all duration-300 border border-camture-rose border-opacity-30">
                                    ...
                                    <img src="{{ asset('storage/' . $photo->file_path) }}"
                                         alt="{{ $photo->title ?? 'Foto Camture tanggal ' . $photo->created_at->format('d M Y') }}" {{-- Judul atau fallback --}}
                                         class="w-full h-full object-contain rounded-md group-hover:scale-105 transition-transform duration-300"> {{-- Efek zoom on hover --}}
                                </div>
                                {{-- Judul Foto --}}
                    ...
                    <div class="mt-8">
                        {{ $photos->links() }} {{-- Render link paginasi (menggunakan view pagination default/Tailwind) --}}
                    </div>
                @endif
            </div>
        </div>
