            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="flex flex-col md:flex-row justify-between items-center mb-6 gap-4">
                        ...
                        <a href="{{ route('admin.templates.create') }}" class="flex items-center justify-center w-full md:w-auto bg-camture-rose text-white px-5 py-2 rounded-md hover:bg-camture-rose-hover transition shadow-sm font-medium">
                    ...
                    <div class="overflow-x-auto border rounded-lg">
                        <table class="w-full text-left">
                            <thead class="bg-gray-50 border-b-2 border-gray-200 text-gray-600 uppercase text-xs tracking-wider">
                                <tr>
                                    {{-- Kolom Preview (Tidak Sortable) --}}
                                    <th class="py-3 px-4 w-1/4">Preview</th>
                                    
                                    {{-- Kolom Nama (Sortable) --}}
                                    <th class="py-3 px-4 font-semibold cursor-pointer hover:bg-gray-100 transition group">
                                           ...
                                           class="flex items-center justify-between group-hover:text-gray-900">
                                            ...
                                            @if(request('sort') == 'name')
                                                @if(request('direction') == 'asc')
                                                    <svg class="w-4 h-4 text-camture-rose" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M5 15l7-7 7 7"></path></svg>
                                                @else
                                                    <svg class="w-4 h-4 text-camture-rose" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 9l-7 7-7-7"></path></svg>
                                                @endif
                                            @else
                                                <svg class="w-4 h-4 text-gray-300 group-hover:text-gray-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 9l4-4 4 4m0 6l-4 4-4-4"></path></svg>
                                            @endif
                                        </a>
                                    </th>

                                    {{-- Kolom Slots (Sortable) --}}
                                    <th class="py-3 px-4 text-center font-semibold cursor-pointer hover:bg-gray-100 transition group">
                                        <a href="{{ route('admin.templates.index', ['sort' => 'capture_slots', 'direction' => request('sort') == 'capture_slots' && request('direction') == 'asc' ? 'desc' : 'asc', 'search' => request('search')]) }}" 
                                           class="flex items-center justify-center group-hover:text-gray-900 gap-1">
                                            ...
                                            @if(request('sort') == 'capture_slots')
                                                <span class="text-camture-rose">{{ request('direction') == 'asc' ? '▲' : '▼' }}</span>
                                            @else
                            ...
                            <tbody class="divide-y divide-gray-100">
                                @forelse ($templates as $template)
                                    <tr class="hover:bg-gray-50 transition">
                                        <td class="py-3 px-4">
                                            <img src="{{ asset('storage/' . $template->image_path) }}" alt="{{ $template->name }}" class="h-16 w-auto rounded-md object-cover border bg-gray-100">
                                        </td>
                                        ...
                                        <td class="py-3 px-4 text-center">
                                            <div class="flex items-center justify-center gap-2">
                                                {{-- Toggle --}}
                                                <form action="{{ route('admin.templates.toggleStatus', $template) }}" method="POST">
                                                    @csrf
                                                    @method('PATCH')
                                                    <button type="submit" title="{{ $template->is_active ? 'Nonaktifkan' : 'Aktifkan' }}" class="{{ $template->is_active ? 'text-yellow-600 hover:text-yellow-800' : 'text-green-600 hover:text-green-800' }}">
                                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                                                ...
                                                {{-- Edit --}}
                                                <a href="{{ route('admin.templates.edit', $template) }}" class="text-blue-600 hover:text-blue-800" title="Edit">
                                                    <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
